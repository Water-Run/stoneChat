...
/* sc_resolve_host_ip($host)
 *   Prefer a numeric IPv4 for stunnel connect=. On some guest
 *   networks (VMware NAT + host fake-IP DNS) stunnel's own
 *   resolver returns a TUN address that cannot complete TLS,
 *   while PHP gethostbyname() honors the system hosts file and
 *   returns a usable address. Leave pure IPs and unresolved
 *   names unchanged. */
if (!function_exists('sc_resolve_host_ip')) {
    function sc_resolve_host_ip($host) {
...
if (!function_exists('sc_generate_stunnel_conf')) {
    function sc_generate_stunnel_conf($conf_path, $pid_path, $ca_cert,
        ...
        $body .= "output = " . $log_path . "\n"
              ...
              . "verify = 2\n";
        if ($tls_host !== '' && !$is_ip) {
            $body .= "checkHost = " . $tls_host . "\n"
                  .  "sni = " . $tls_host . "\n";
        }
        $body .= "accept = 127.0.0.1:" . (int)$local_port . "\n"
              . "connect = " . $connect_host . ":" . (int)$remote_port . "\n";
        $bytes = @file_put_contents($conf_path, $body);
        return ($bytes !== false);
    }
}

/* sc_win_stunnel_launcher_body($stunnel_exe, $conf_path)
 *   Build a Windows Script Host launcher that starts stunnel hidden and
 *   returns immediately. PHP's exec("start /B ...") keeps inherited console
 *   handles open on XP and can block the API request until stunnel exits. */
if (!function_exists('sc_win_stunnel_launcher_body')) {
    function sc_win_stunnel_launcher_body($stunnel_exe, $conf_path) {
        $command = '"' . $stunnel_exe . '" "' . $conf_path . '"';
        $vbs_command = str_replace('"', '""', $command);
...
if (!function_exists('sc_pid_alive')) {
    function sc_pid_alive($pid) {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            ...
            $tasklist = sc_win_sys_cmd('tasklist.exe');
...
/* sc_win_sys_cmd($name)
 *   Absolute path to a system executable under %SystemRoot%\system32.
 *   PHP -S under Windows XP often has a PATH that omits system32, so
 *   bare "netstat"/"tasklist" calls fail even though the tools exist. */
if (!function_exists('sc_win_sys_cmd')) {
    function sc_win_sys_cmd($name) {
        $root = getenv('SystemRoot');
        if (!is_string($root) || $root === '') {
            $root = 'C:\\WINDOWS';
        }
        return rtrim($root, '\\/') . '\\system32\\' . $name;
    }
}

/* sc_stunnel_port_open($port)
 *   True when something accepts TCP on 127.0.0.1:$port. Used as a
 *   reliable readiness probe that does not depend on netstat/PATH. */
if (!function_exists('sc_stunnel_port_open')) {
    function sc_stunnel_port_open($port) {
        $port = (int)$port;
        if ($port <= 0) {
            return false;
        }
...
if (!function_exists('sc_stunnel_port_pid')) {
    function sc_stunnel_port_pid($port) {
        ...
        $output = array();
        $rc = 0;
        $netstat = sc_win_sys_cmd('netstat.exe');
        @exec('"' . $netstat . '" -ano -p tcp 2>NUL', $output, $rc);
        if (!is_array($output) || count($output) === 0) {
            /* fallback: bare name, for environments with a full PATH */
            @exec('netstat -ano -p tcp 2>NUL', $output, $rc);
        }
        if (!is_array($output) || count($output) === 0) {
            return 0;
...
if (!function_exists('sc_stunnel_start')) {
    function sc_stunnel_start($stunnel_exe, $conf_path, $pid_path) {
        ...
        if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
            ...
            if (is_file($tstunnel)) {
                $stunnel_exe = $tstunnel;
            }
            $launcher = dirname($conf_path) . DIRECTORY_SEPARATOR
                      . 'stunnel-launch.vbs';
            $launcher_body = sc_win_stunnel_launcher_body(
                $stunnel_exe, $conf_path
            );
            if (@file_put_contents($launcher, $launcher_body) === false) {
                return false;
            }
            $wscript = sc_win_sys_cmd('wscript.exe');
