...
PHP 5.4 or newer is required. The launcher uses PHP's built-in web
server:
...
#+begin_src ini
[User Guest]
password = REPLACE_WITH_GUEST_PASSWORD
active = true
...
Replace the sample passwords before launching. The name after ~User~ is
the display name shown on the page. It does
...
- The compatibility banner is written to render in IE11 and closes
  itself after three seconds.
- Markdown rendering is intentionally small and local; no browser
