<?php
/* -------------------------------------------------------------------------
 * stoneChat / Server/api/providers.php
 *
 * Routes:
 *   GET  /api/providers
 ...
 *
 * Security notes:
 *   - The full api_key is never written to the response. The public
 *     shape is always "****" (non-empty key) or "" (empty). The
 *     sc_api_providers_mask_key() helper is kept for callers that
 *     explicitly want a less-aggressive redaction.
 *   - test_all results include id/ok/latency_ms/error only; no
 *     secrets.
 *
 * PHP 5.2 compatible.
 * ------------------------------------------------------------------------- */
...
/* sc_api_providers_mask_key($key)
 *   Mask an api_key for safe display. "XXXX****YYYY" for keys at
 *   least 12 bytes long; "****" otherwise. The full key is never
 *   returned. */
...
/* sc_api_providers_is_placeholder_key($key)
 *   Heuristic: does the key look like an unfilled CONF.ini stub?
 *   Empty strings, "****", and the "YOUR_*_HERE" pattern are
...
/* sc_api_providers_check_base($base)
...
if (!function_exists('sc_api_providers_normalize')) {
    function sc_api_providers_normalize($p, $defaults) {
        ...
        return array(
            ...
            'model'        => $model,
            'api_base'     => $api_base,
            /* api_key field is exposed only as a redacted marker
             * ("****" for any non-empty key, "" for empty). The
...
if (!function_exists('sc_api_providers_ping_one')) {
    function sc_api_providers_ping_one($provider, $timeout_seconds) {
        ...
        if ($timeout_seconds > 0 && $elapsed_ms > ($timeout_seconds * 1000)) {
            ...
            return array('id' => $id, 'ok' => false,
                         'latency_ms' => $elapsed_ms, 'error' => $err);
        }
        if (!is_array($result)) {
            return array('id' => $id, 'ok' => false,
                         'latency_ms' => $elapsed_ms, 'error' => 'no_result');
        }
        if (empty($result['ok'])) {
        ...
        return array('id' => $id, 'ok' => true,
                     'latency_ms' => $elapsed_ms, 'error' => '');
    }
}

/* sc_api_providers_test_all($raw, $by_id, $per_timeout, $total_timeout)
 *   Ping every configured provider; cap total wall-clock at
 *   $total_timeout. Providers marked unavailable during normalize
 *   are skipped (their precomputed 'reason' is forwarded). */
if (!function_exists('sc_api_providers_test_all')) {
    function sc_api_providers_test_all($raw, $by_id, $per_timeout,
                                       $total_timeout) {
        $out = array();
        $deadline = ($total_timeout > 0)
                    ? (microtime(true) + (float)$total_timeout) : 0.0;
        if (!is_array($raw)) {
            return $out;
        }
        foreach ($raw as $p) {
            $id = (is_array($p) && isset($p['id']))
                  ? (string)$p['id'] : '';
            if (!empty($deadline) && microtime(true) >= $deadline) {
                $out[] = array('id' => $id, 'ok' => false,
                               'latency_ms' => 0, 'error' => 'timeout');
                continue;
            }
            if (isset($by_id[$id]) && is_array($by_id[$id])
                && empty($by_id[$id]['available'])) {
                $reason = isset($by_id[$id]['reason'])
                          ? (string)$by_id[$id]['reason'] : 'unavailable';
                $out[] = array('id' => $id, 'ok' => false,
                               'latency_ms' => 0, 'error' => $reason);
                continue;
            }
