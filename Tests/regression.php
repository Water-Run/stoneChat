...
$test = 'router delegates validated non-PHP Pages assets to PHP static serving';
if (is_string($router_text)) {
    $pages_start = strpos($router_text, "if (strpos(\$path, '/Pages/') === 0)");
    $pages_end = strpos($router_text, '/* 3) Server-side PHP endpoints. */');
    $pages_branch = ($pages_start === false || $pages_end === false)
                  ? ''
                  : substr($router_text, $pages_start, $pages_end - $pages_start);
    sc_test_assert_true(
        $test,
        preg_match(
            "/if \\(\\\$ext === '\\.php'\\) \\{.*?return true;\\s*\\}\\s*return false; \\/\\* PHP's static server serves validated Pages assets\\. \\*\\//s",
            $pages_branch
        ) === 1,
        'validated non-PHP Pages files should delegate to PHP static serving'
    );
    sc_test_assert_true(
        $test,
        strpos($pages_branch, 'readfile($file)') === false,
        'validated non-PHP Pages files should not be read manually'
    );
}
